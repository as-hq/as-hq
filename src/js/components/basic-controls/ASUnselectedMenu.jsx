...
A note on the focus problem.

Focus is not returned to the SS upon clicking 'insert row'
The reason is that Menu.state.focusIndex defaults to 0, even when the Menu
doesn't actually have a value / isn't a dropdown menu.
To fix this, we created our own version of Menu, which doesn't let focusIndex
default to 0, and keeps it -1 even when the menu is open.
...
import Controllable from 'material-ui/lib/mixins/controllable';
...
const Menu = React.createClass({
  ...
  _handleMenuItemTouchTap(e, item) {
    ...
    // AS: remove let focusIndex = React.isValidElement(children) ? 0 : children.indexOf(item);

    // AS: remove this._setFocusIndex(focusIndex, false);
