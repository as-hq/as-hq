...
-- timchu, 12/17/15.
conditionallyFormatCells :: KernelAddress -> Connection -> ASSheetId -> [ASCell] -> [CondFormatRule] -> EvalContext -> EitherTExec [ASCell]
conditionallyFormatCells sett conn origSid cells rules ctx = do
  let cells' = map (cellProps %~ clearCondFormatProps) cells
      transforms = map (ruleToCellTransform sett conn origSid ctx) rules
...
ruleToCellTransform state conn sid ctx cfr@(CondFormatRule _ rngs condFormatCondition format) c@(Cell l e v ps rk) = do
  let containingRange = find (flip rangeContainsIndex l) rngs
  case containingRange of
    Nothing -> return c
    Just rng -> do
      let tl = getTopLeft rng
          eval = evaluateExpression sett conn sid ctx
...
evaluateExpression state sid ctx xp@(Expression str lang) = do
  ...
  (Formatted res _) <- evaluateLanguage sett conn dummyLoc ctx' xp
  case res of
