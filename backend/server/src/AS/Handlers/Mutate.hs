...
-- Functions from colValueMutate and rowValueMutate are helper functions for barIndexMutate,
--
...
-- colValueMutate and rowValueMutate should change. This code as is is a temporary hack.
-- Additionally, there is a lot of code dulication between methods on Col and Row.
colValueMutate :: MutateType -> Int -> Maybe Int
colValueMutate (InsertCol c') c = Just $ if c >= c' then (c+1) else c
colValueMutate (DeleteCol c') c
  | c == c'  = Nothing
  | c > c'   = Just $ c-1
  | c < c'   = Just c
colValueMutate (DragCol oldC newC) c -- DragCol 3 1 : (123) -> (312)
...
colValueMutate _ c = Just c

--TODOX: Timchu. It is odd that colValueMutate is used outside of the below function.
--This is a signal that types need to change.
colMutate ::  MutateType -> Col -> Maybe Col
colMutate mt col = col & int %%~ colValueMutate mt

rowValueMutate :: MutateType -> Int -> Maybe Int
rowValueMutate (InsertRow r') r = Just $ if r >= r' then (r+1) else r
rowValueMutate (DeleteRow r') r
  | r == r'  = Nothing
  | r >  r'   = Just $ r-1
  | r <  r'   = Just r
rowValueMutate (DragRow oldR newR) r -- DragRow 3 1 : (123) -> (312)
...
rowValueMutate _ r = Just r

rowMutate :: MutateType -> Row -> Maybe Row
rowMutate mt row = row & int %%~ rowValueMutate mt
...
barIndexMutate mt bar@(BarIndex sid typ bari) = do
  bari' <- case typ of
             ColumnType -> colValueMutate mt bari
             RowType -> rowValueMutate mt bari
...
coordMutate mt (Coord col row) = do
  col' <- col & int %%~ (colValueMutate mt)
  row' <- row & int %%~ (rowValueMutate mt)
...
exIndexMutate mt = (exCol %%~ (exColMutate mt)) >=> (exRow %%~ (exRowMutate mt))

exColMutate :: MutateType -> ExCol -> Maybe ExCol
exColMutate mt exCol = exCol & ind %%~ colMutate mt

exRowMutate :: MutateType -> ExRow -> Maybe ExRow
exRowMutate mt exRow = exRow & ind %%~ rowMutate mt
...
shiftExIndexUp exIndex = fromJust $ (shiftExIndexForced $ Offset 0 (-1)) exIndex

shiftExIndexLeft :: ExIndex -> ExIndex
shiftExIndexLeft exIndex = fromJust $ (shiftExIndexForced $ Offset (-1) 0) exIndex

shiftExColLeft :: ExCol -> ExCol
shiftExColLeft = fromJust . (shiftExIntForced (-1))
...
exColRangeMutate mt exColRange =
  ...
  case (maybeTl, maybeR) of
    ...
    (Just tl', Nothing) -> Just $ ExColRange tl' (shiftExColLeft r) -- in this case, tl' should equal tl
...
fatCellGotMutated (InsertCol c) (RangeKey (Index _ coord) dims) = between (tlc + 1) (tlc + (width dims)^.int - 1) c
  where tlc = coord^.col^.int
fatCellGotMutated (InsertRow r) (RangeKey (Index _ coord) dims) = between (tlr + 1) (tlr + (height dims)^.int - 1) r
  where tlr = coord^.row^.int
fatCellGotMutated (DeleteCol c) (RangeKey (Index _ coord) dims) = between tlc (tlc + (width dims)^.int - 1) c
  where tlc = coord^.col^.int
fatCellGotMutated (DeleteRow r) (RangeKey (Index _ coord) dims) = between tlr (tlr + (height dims)^.int - 1) r
  where tlr = coord^.row^.int
...
fatCellGotMutated (DragCol c1 c2) (RangeKey (Index _ coord) dims) = case (width dims) of
  ...
  _ -> or [
      between tlc (tlc + (width dims) - Col 1) $ Col c1
    , between (tlc + Col 1) (tlc + (width dims) - Col 1) $ Col c2
    ]
  where tlc = coord^.col
...
fatCellGotMutated (DragRow r1 r2) (RangeKey (Index _ coord) dims) = case (height dims) of
  ...
  _ -> or [
      between tlr (tlr + (height dims) - Row 1) $ Row r1
    , between (tlr + Row 1) (tlr + (height dims) - Row 1) $ Row r2
    ]
  where tlr = coord^.row
...
-- Question, timchu, 12/29/15. We seem to allow improperly oriented ranges!
  ...
  | tlCol == Col c && blCol == Col c = []
  | tlCol == Col c             = [Range sid (Coord tlCol tlRow, Coord (blCol-Col 1) blRow)] -- not (Coord tlCol+1 tlRow, Coord blCol blRow) since the cols gets shifted
  | blCol == Col c             = [Range sid (Coord tlCol tlRow, Coord (blCol-Col 1) blRow)]
...
rangeMutate mt@(DeleteRow r) rng@(Range sid (Coord tlCol tlRow, Coord blCol blRow))
  ...
  | tlRow == Row r && blRow == Row r = []
  | tlRow == Row r             = [Range sid (Coord tlCol tlRow, Coord blCol (blRow-Row 1))]
  | blRow == Row r             = [Range sid (Coord tlCol tlRow, Coord blCol (blRow-Row 1))]
  | otherwise            = rangeMutate' mt rng
