...
data Update a b = Update { _newVals :: [a], _oldKeys :: [b] } deriving (Show, Read, Eq, Data, Typeable, Generic)
makeLenses ''Update
...
filterUpdateByKey f update =
  update & newVals %~ filter (f . key) & oldKeys %~ filter f
...
addValue d update = if (inRemoved $ key d) 
  then update & oldKeys %~ L.delete (key d)
  -- TODO: tim. What the hell is d????
  else update & newVals .~ d:(update^.newVals)
    where inRemoved x = x `L.elem` (update^.oldKeys)

removeKey :: (HasKey a, Eq (KeyType a)) => (KeyType a) -> Update a (KeyType a) -> Update a (KeyType a)
removeKey d update = if (inAdded d)
    then update & newVals %~ L.filter ((/=) d . key)
  else update & oldKeys .~ d:(update^.oldKeys)
    where inAdded x = x `L.elem` (map key $ update^.newVals)
